...
static void super_create(unsigned int nblocks, unsigned int ninodeblocks) {
  ...
  ffs_IMsb.sb.ninodes = (ninodeblocks/2) * SML_INOS_PER_BLK + (ninodeblocks/2) * LRG_INOS_PER_BLK;
...
static int super_read() {
  ...
  ercode=disk_ops.read(/*** TODO ***/0, sb_u.data);
  if (ercode < 0) return ercode;

  memcpy(&ffs_IMsb.sb, &sb_u.sb, sizeof(struct super)); //DISK_BLOCK_SIZE manda erro de overflow???
...
static int super_write() {
  ...
  memcpy(&sb_u.sb, &ffs_IMsb.sb/*** TODO ***/, DISK_BLOCK_SIZE/*** TODO ***/);

  ercode=disk_ops.write/*** TODO ***/(/*** TODO ***/0, /*** TODO ***/sb_u.data);
  if (ercode < 0) return ercode;
...
static int super_mount( char *diskname, int debug ) {
  ...
  ercode = super_read(); //rever, presumo que seja isto
  if (ercode < 0) return ercode;
