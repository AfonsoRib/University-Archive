...
static int file_create_C( char *name, unsigned int nblocks, char type) {
  ...
  /*** TODO ***/ ercode = bmap_ops.getfree(DATA_BMAP,nblocks);
  if (ercode < 0) return ercode;
  unsigned int dm2set=ercode;

  // get the inode entry for this file
  /*** TODO ***/ ercode = bmap_ops.getfree(SML_INODE_BMAP, 1);
  if (ercode < 0) return ercode;
  unsigned int inode2set=ercode;

  // create the dentry
  /*** TODO ***/ ercode = dir_ops.create(name,inode2set);
  if (ercode < 0) return ercode;

  // mark the disk block entry for this file
  /*** TODO ***/bmap_ops.set(DATA_BMAP, dm2set,nblocks,1);
  if (ercode < 0) return ercode; // This would be a bug!

  // mark the inode bmap entry for this file
  /*** TODO ***/ bmap_ops.set(SML_INODE_BMAP, inode2set, 1, 1);
  if (ercode < 0) return ercode; // This would be a bug!
  ...
  /*** TODO ***/inode_ops.read(inode2set, &ino);
  if (ercode < 0) return ercode; // This would be a bug!
  ...
  // Save the inode 
  /*** TODO ***/inode_ops.write(inode2set, &ino);
  if (ercode < 0) return ercode; // This would be a bug!
...
static int file_create_I(char *name) {
  ...
  // get the inode entry for this file
  /*** TODO ***/ ercode = bmap_ops.getfree(LRG_INODE_BMAP, 1);
  if (ercode < 0) return ercode;
  unsigned int inode2set=ercode;

  // create the dentry
  /*** TODO ***/ dir_ops.create(name, inode2set);
  if (ercode < 0) return ercode;

  // mark the inode bmap entry for this file
  /*** TODO ***/ bmap_ops.set(LRG_INODE_BMAP, inode2set, 1, 1); 
  if (ercode < 0) return ercode; // This would be a bug!
